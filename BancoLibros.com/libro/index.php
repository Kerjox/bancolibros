...
session_start();
	   ...
	   //Ver si hay algo almacenado en la variable session
        ...
        if (!isset($_SESSION["usuario"])) {       
			$boton_login = "<a href='/login/?location=libro&id=$_GET[id]' class='btn btn-outlined'>Log in <i class='fas fa-sign-in-alt'></i></a>";
			$contact = "";
       }else{
		   	$contact = "
		<div class='span-3-25'>
		<form method='post' action='./mensaje/sendMensaje.php?para=$valores[IDusuario]&libro=$valores[IDlibro]'>
		<div class='fields'>
	   <div class='field'>
						...
						<!-- Panel (Banner) -->
							<section class="panel banner right">
								<div class="content color0 span-3-75">
                                    <h1 class="major"><?php echo "$valores[Titulo]" ?></h1>
									<p><?php echo"$valores[Comentarios]" ?></p>
									<ul class="actions">
                                        <li><span class="button primary color2 disabled"><?php echo"$valores[Precio]" ?><i class="fas fa-euro-sign"></i></span></li>
                                        <li><a href="#first" class="button primary color1 circle icon solid fa-angle-right">Next</a></li>
									</ul>
								</div>
								<div class="image filtered span-1-75" data-position="25% 25%">
                                    <img src="/img/libros/<?php echo"$valores[Foto]" ?>" alt="" />
								</div>
							</section>
						...
						<!-- Panel (Spotlight) -->
							<section class="panel spotlight left">
								<div class="content span-5">
                                    <h2 class="major">Información del vendedor</h2>
                                    <div class="table-wrapper">
											<table class="alt">
                                                ...
                                                <tbody>
                                                <?php
                $aql = $conn->query("SELECT Nombre, Apellido, Movil, email FROM usuarios as u WHERE DNI=$valores[IDusuario]");

                while ($usuario = mysqli_fetch_array($aql)) {
                    echo "<tr>
						...
						<!-- Panel -->
							<section class="panel color4-alt" id="first">
								<div class="intro color4">
									...
									<p>Aquí puedes puedes mandarle un mensaje al vendedor por si estás interesado en el libro.</p>
									<p><?php echo "$boton_login" ?></p>
								</div>
